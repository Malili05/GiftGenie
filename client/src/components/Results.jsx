...
const Results = () => {
  ...
  const [displayedGift, setDisplayedGift] = useState(null); // Initialize state for displayed gift

  useEffect(() => {
    if (!loading && !error && data && data.gifts.length > 0) {
      // Set the displayed gift when data is loaded
  ...
  const handleSaveGift = (giftId) => {
    console.log("Saving gift with ID:", giftId); // Log the giftId
    console.log("Gift to be saved:", displayedGift); // Log all the gift
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen text-center">
      <div className="results-container bg-gray-100 p-5 rounded-lg shadow-md w-full max-w-md mx-auto">
        {displayedGift && (
          <GiftDisplay
            key={displayedGift._id}
            gift={displayedGift}
            handleSaveGift={() => handleSaveGift(displayedGift._id)}
            isSaved={isSaved}
            saveError={saveError}
          />
        )}
      ...
      {AuthService.loggedIn() && (
        <>
          <button
            ...
            className="block mt-4 mx-auto px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-700 transition duration-300"
          >
            SAVE THIS GIFT
          </button>
          {isSaved && (
            <p className="text-green-500 mt-2">Gift saved successfully!</p>
          )}
          {saveError && (
            <p className="text-red-500 mt-2">Failed to save the gift. Please try again later.</p>
          )}
        </>
      )}
    </div>
